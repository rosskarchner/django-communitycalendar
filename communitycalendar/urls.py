...
urlpatterns = patterns('communitycalendar.views',
    ...
    url(r'^groups/(?P<slug>[-_\w]+)/$', GroupDetail.as_view(),
        name='group-detail'),
    url('^groups/(?P<slug>[-_\w]+)/edit/$', GroupUpdate.as_view(),
        name='group-update'),
    url('^groups/(?P<slug>[-_\w]+)/delete/', GroupDelete.as_view(),
        name='group-delete'),
    url('^groups/$', GroupList.as_view(),
        name='group-list'),
    url('^groups/(?P<slug>[-_\w]+)/apply-to-organize/',
        ApplyToOrganize.as_view(),
        name='apply-to-organize'),
    url('^groups/(?P<slug>[-_\w]+)/invite-to-organize/',
        InviteToOrganize.as_view(),
        name='invite_to_organize'),
    url('^groups/(?P<slug>[-_\w]+)/add-an-event/$',
